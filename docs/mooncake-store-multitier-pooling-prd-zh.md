...
- Mooncake Store 在“内存池 + NoF SSD 池”场景下的产品需求边界。
- 多级池化对对象写入、读取、删除、淘汰、故障隔离和恢复的能力要求。
- 面向推理服务连续性的非功能需求，包括性能、可靠性、可运维性和可测试性。
- MVP、v1、v2 的需求优先级划分是否合理。

本次评审不覆盖以下内容：

- SPDK/NoF target 的具体实现细节和代码设计。
- 具体算法实现，如热度预测模型、复杂预取算法和高级迁移算法。
- 跨地域复制、长期归档和数据库级持久化。
...
- 上层推理服务已经具备 cache miss 后的回源重建能力。
- NoF SSD 层可以作为容量扩展层参与对象访问，但其可靠性目标低于一级内存层。
- 系统允许在故障场景下通过退化、回源和淘汰来维持服务。
- 运维团队可以接入并维护分层监控、告警和配置变更流程。

### 5.2 约束条件

- 不新增强持久化或强一致性语义。
- 不要求上层业务主动参与分层决策。
- 不要求在所有场景下维持与纯内存方案完全相同的延迟表现。
- 需求中的定量指标在本轮以 `TBD` 表示，待 workload 基线补齐后固化。
...
- **验收口径**：
  - 对象写入后至少存在一级层可读副本。
  - 二级层可用时，对象可按策略获得 NoF 副本。
  - 容量不足场景优先触发淘汰，再决定退化或失败。
...
- **评审关注点**：
  - 当前测试计划是否已覆盖四类路径。
  - 是否存在无法在 CI/测试环境验证的硬件依赖项。

## 11. 依赖、风险与开放问题

### 11.1 外部依赖

- NoF SSD target 与接入环境可用。
- 控制面能够获得可靠的分层状态和健康检查结果。
- 监控系统可承载新增分层指标和告警。
- 上层推理框架可正确处理 cache miss 和退化场景。
...
- 可观测性不足会放大排障难度。
- 淘汰策略不合理可能导致命中率抖动或过度回源。
- 定量指标未落地前，评审结论更偏方向性而非封版结论。

### 11.3 开放问题

- 内存层和 NoF 层的默认淘汰策略是否一致，还是应分层定义。
- NoF 层不可用时，是否需要限制新对象写入速率以保护内存层。
- 是否需要为不同对象类型定义不同分层和淘汰优先级。
- `TBD` 指标应由哪类 workload 和基线数据来最终固化。
...
- 是否接受“推理连续性优先于缓存强恢复”作为可靠性边界。
- 是否接受 NoF SSD 层承担温数据和扩容层角色，而非强持久化层。
- 是否接受淘汰作为容量控制主机制之一，并进入写入异常流。
- 是否认可以当前 use case 归纳出的 6 类核心系统能力。
- 是否认可以 `FR-1` 到 `FR-9` 作为本轮功能需求基线。
- 是否认可以 `NFR-1` 到 `NFR-6` 作为本轮非功能需求基线。
- 哪些 `TBD` 指标必须在立项/设计冻结前补齐，哪些可延后到 v1。
- MVP 是否只要求“可运行、可降级、可观测”，而不追求高级智能策略。
